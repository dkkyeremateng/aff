...
func TestGoodLuck(t *testing.T) {
	tests := []struct {
		name  string
		input string
		want  string
	}{
		{name: "direct name", input: "Ada", want: "Good luck, Ada!"},
		{name: "whitespace-padded name", input: " Ada ", want: "Good luck, Ada!"},
		{name: "empty name", input: "", want: "Good luck!"},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := GoodLuck(tt.input)
			if got != tt.want {
				t.Errorf("GoodLuck(%q) = %q, want %q", tt.input, got, tt.want)
			}
		})
	}
}
